...
object Game {
  ...
  def apply(id: String, playersNumber: Int = 1, visible: Boolean = true): Behavior[GameCommand] = {
    ...
    Behaviors.setup { context =>
      ...
      new Game(Setup(id, context, engine, playersNumber)).start(Model(Set(), ghosts))
    }
  }
  ...
  private def initRoutine(model: Model): Behaviors.Receive[Game.GameCommand] =
    Behaviors.receiveMessage {
      ...
      case RegisterPlayer(replyTo, source) =>
        ...
        val player = setup.context.spawn(Player(setup.id, setup.engine), s"PlayerActor")
        player ! Player.RegisterUser(replyTo, source)

        //TODO  if numPl == registerPlayer core else init
