...
case class ShortestPath(graph: Graph, tileStart: GraphVertex, tileEnd: GraphVertex) extends TermConvertible {
  /**
   * Something like "shortest_path([ t0-[t1-1], t1-[t2-1, t3-1], t2-[], t3-[] ], t0, t3, X)"
   */
...
case class ShortestPathClassic(tileStart: GraphVertex, tileEnd: GraphVertex) extends TermConvertible {
  /**
   * Something like "shortest_path(t(6,1), t(15,29), X)" on classic pacman map
   */
  override def toTerm: Term = new Struct("shortest_path", new Struct("classic"), tileStart.toTerm, tileEnd.toTerm, new Var())
}
